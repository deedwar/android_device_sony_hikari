...
DEVICE := device/sony/hikari
MKELF := device/sony/hikari/tools/mkelf.py
INITSH := device/sony/hikari/combinedroot/init.sh
...
$(INSTALLED_BOOTIMAGE_TARGET): $(PRODUCT_OUT)/kernel $(uncompressed_ramdisk) $(recovery_uncompressed_ramdisk) $(INSTALLED_RAMDISK_TARGET) $(INITSH) $(BOOTREC_DEVICE) $(PRODUCT_OUT)/utilities/busybox $(PRODUCT_OUT)/utilities/extract_elf_ramdisk $(MKBOOTIMG) $(MINIGZIP) $(INTERNAL_BOOTIMAGE_FILES)
	...
	$(hide) cp $(DEVICE)/bootlogo/logo.rle $(PRODUCT_OUT)/root/logo.rle
	...
	$(hide) python $(MKELF) -o $@ $(PRODUCT_OUT)/kernel@0x40208000 $(PRODUCT_OUT)/combinedroot.fs@0x41500000,ramdisk vendor/sony/hikari/proprietary/boot/RPM.bin@0x20000,rpm
...
$(INSTALLED_RECOVERYIMAGE_TARGET): $(MKBOOTIMG) \
	...
	$(hide) python $(MKELF) -o $@ $(PRODUCT_OUT)/kernel@0x40208000 $(PRODUCT_OUT)/ramdisk-recovery.img@0x41500000,ramdisk vendor/sony/hikari/proprietary/boot/RPM.bin@0x20000,rpm
